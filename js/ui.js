...
async function acceptFiles(fileList, preferred) {
  // Hasta tres: in-sample, forward y el informe del backtest del periodo no visto.
...
function changeLanguage(lang) {
  ...
  syncLangButtons();
  refreshAnalyzeButton();
  // Los hallazgos del motor se generan en el idioma activo: si hay datos, se rehace.
  if (state.isTable && !state.busy) runAudit();
  else render();
}
...
async function analyseOnMainThread(payload) {
  ...
  return runAnalysis({ isTable, oosTable, policy: payload.policy, onProgress: (p) => showProgress(p.pct, p.label) });
}
...
function verdictCopy(level) {
  ...
  if (level === 'weak') return { label: t('verdict.weak'), cls: 'v-no' };
...
function renderDiagnostics(a) {
  ...
  return `<div class="detail-head">
    ...
    <div class="grid-secondary">
      ...
      <section class="panel">
        ...
        <div class="evidence-list">
          ...
          <div><span>${L('Cobertura', 'Coverage')}</span><strong>${Number.isFinite(a.meta.coverage) ? nf(5).format(a.meta.coverage * 100) + ' %' : '—'}</strong></div>
    ...
    <section class="panel">
      ...
      <p class="chart-note">
        ${L(
          `<strong>Los dos umbrales del Sharpe.</strong> El contraste publicado (Bailey y López de Prado) usa como
        ...
        que la distancia entre ellos la juzgues tú.`,
          `<strong>The two Sharpe thresholds.</strong> The published contrast (Bailey and López de Prado) uses as
        ...
        you can judge the distance between them.`,
        )}
      </p>
      <p class="chart-note">
        ${L(
