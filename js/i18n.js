...
const STRINGS = {
  en: {
    ...
    'lp.cta.footer': 'App',
    'lp.demo': 'See a sample report',
    'lp.proof': 'Includes a synthetic example that shows how Orometra tells a stable plateau from a lucky peak — open <strong>See a sample report</strong> to explore it without uploading any files.',
    ...
    'lp.what.2.body': 'Three concrete configurations, neighbor support, out-of-sample behavior, and the risks that still apply.',
    ...
    'lp.avoid.1.body': 'The top of an MT5 table is often an isolated peak. Neighbors and out-of-sample behavior stay invisible until you audit the surface.',
    ...
    'lp.not.4': 'It is most precise on a full grid; with genetic/sparse search it gets more conservative, and says so.',
    ...
    'doc.method.s5.body': 'The optimization grid only has aggregated per-pass metrics — never enough for a real Monte Carlo. Your chosen configuration\'s backtest report on the unseen period does carry the trade-by-trade list, and that unlocks three checks the grid alone cannot: a Monte Carlo over your real trades, a sample-size check, and a cost-stress table for worse-than-optimized spread or slippage. Drop that report — a real backtest, not an optimization export — on the “Unseen period” step.',
  ...
  es: {
    ...
    'lp.proof': 'Incluye un ejemplo con datos sintéticos que muestra cómo Orometra separa una meseta estable de un pico de suerte — abre <strong>Ver un informe de ejemplo</strong> para explorarlo sin subir ningún archivo.',
    ...
    'lp.not.4': 'Es más preciso con rejilla completa; con genético/muestreo disperso se vuelve más cauto, y lo dice.',
    ...
    'doc.method.s4.body': 'El veredicto es sólido / moderado / débil / insuficiente. Nunca significa «opera». Pocos datos no es lo mismo que «no hay meseta».',
    'doc.method.s5.title': '5. Puesta a prueba con tus operaciones reales',
    'doc.method.s5.body': 'La rejilla de optimización solo trae métricas agregadas por pasada — nunca alcanza para un Monte Carlo de verdad. El informe de backtest de tu configuración elegida sobre el periodo no visto sí trae la lista de operaciones una a una, y eso desbloquea tres comprobaciones que la rejilla sola no puede darte: un Monte Carlo sobre tus operaciones reales, un aviso de tamaño de muestra, y una tabla de stress de costes para cuando el spread o el slippage en vivo sean peores que los usados al optimizar. Suelta ese informe — un backtest real, no un export de optimización — en el paso «Periodo no visto».',
