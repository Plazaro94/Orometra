...
| SR-1 | El «Sharpe Ratio» del export de optimización es **por operación** (no anualizado / no por barra de equity) | Error típico del contraste Lo y umbrales de azar | **Verificado como FALSO** (búsqueda en documentación MetaQuotes) | Desde el build 3210 del terminal (11 feb 2022), MT5 calcula el Sharpe sobre los log-retornos del equity **por barra**, anualizados a un intervalo de un año, con tasa libre de riesgo = 0 — no por operación. El contraste Lo del panel de Diagnósticos ya avisa de esto explícitamente (ver `js/ui-plateaus.js`, nota "Como leer el contraste del Sharpe"); pendiente decidir si se corrige el error típico en `core/verdict.js`'s `sharpeTest` o se deja solo como advertencia de texto |
